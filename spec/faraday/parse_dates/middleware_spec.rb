...
RSpec.describe Faraday::ParseDates::Middleware, type: :response do
  let(:parsed) { Time.utc(2012, 2, 1, 13, 14, 15) }

  it 'parses dates' do
    expect(process('x' => '2012-02-01T13:14:15Z').body['x']).to eq(parsed)
    expect(process('x' => '2012-02-01T15:14:15+02:00').body['x'].utc).to eq(parsed)
    expect(process('x' => '2012-02-01T11:14:15-0200').body['x'].utc).to eq(parsed)
  end
  ...
  it 'parses nested dates in hash' do
    expect(process('x' => { 'y' => '2012-02-01T13:14:15Z' }).body['x']['y']).to eq(parsed)
  end

  it 'parses nested dates in arrays' do
    expect(process('x' => [{ 'y' => '2012-02-01T13:14:15Z' }]).body['x'][0]['y']).to eq(parsed)
  end
  ...
  it 'does not parse date-like things' do
    expect(process('x' => '2012-02-01T13:14:15Z bla').body['x']).to eq '2012-02-01T13:14:15Z bla'
    expect(process('x' => '12012-02-01T13:14:15Z').body['x']).to eq '12012-02-01T13:14:15Z'
    expect(process('x' => "2012-02-01T13:14:15Z\nfoo").body['x']).to eq "2012-02-01T13:14:15Z\nfoo"
  end
end
